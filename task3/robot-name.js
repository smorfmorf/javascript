export class Robot {
  ...
  // В них можно делать какие-то общие данные. Основная суть статических методов и свойств в том, что они привязаны не к конкретным экземплярам класса, а к самому классу.
  static usedNames = new Set();
  constructor() {
    // // Привязано к самому классу Robot, и чтобы поле не перезаписывалось указываем условие.
    // if (!static.usedNames) {
    //   // общее свойство (когда данные должны быть общие для всех объектов)
